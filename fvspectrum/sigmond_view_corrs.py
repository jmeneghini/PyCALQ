...
class SigmondPreviewCorrs:
    ...
    def __init__(self, task_name, proj_file_handler, general_params, task_params, sph):
        ...
        if (
            not self.other_params["create_pdfs"]
            and not self.other_params["create_pickles"]
            and not self.other_params["create_summary"]
        ):
        ...
        for channel in self.channels:
            ...
            for op1 in self.data_handler.getChannelOperators(channel):
                ...
                for op2 in self.data_handler.getChannelOperators(channel):
                    ...
                    estimates = sigmond.getEffectiveEnergy(
                        mcobs_handler,
                        corr,
                        self.project_handler.hermitian,
                        self.project_handler.subtract_vev,
        ...
        if not self.other_params["generate_estimates"] and self.other_params["plot"]:
            if self.project_handler.nodes:
                ...
                for channels in channels_per_node:
                    processes.append(
                        Process(
                            target=self.write_channel_plots_data,
                            args=(
                                channels,
                                plh,
                            ),
            ...
            if self.project_handler.nodes:
                ...
                for channels in channels_per_node:
                    processes.append(
                        Process(
                            target=self.write_channel_plots,
                            args=(
                                channels,
                                plh,
                            ),
                ...
                if self.project_handler.nodes:
                    ...
                    for i, psq in enumerate(self.moms):
                        if len(processes) == i:
                            processes.append(
                                Process(
                                    target=plh.compile_pdf,
                                    args=(
                                        self.proj_file_handler.summary_file(psq),
                                        i,
                                    ),
                                )
                            )
