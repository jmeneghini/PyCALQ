...
class Plotter:
    ...
    def phase_shift_plot_error(self, results_file, best_fit_a, best_fit_b, num_points=100, confidence_level=95):
        ...
        y_values = (-1 / (best_fit_a + parameter_deviations[:, 0])) + 0.5 * (
            best_fit_b + parameter_deviations[:, 1]
        ) * q2_values
        ...
        plt.fill_between(
            q2_values,
            np.percentile(y_values, 16, axis=0),
            np.percentile(y_values, 84, axis=0),
            color="lightblue",
            alpha=0.5,
        )
        plt.xlabel("q^2")
    ...
    def plot_energy_histograms_1sigma(self, separate_plots=True):
        ...
        if separate_plots:
            ...
            for data, label, color in datasets:
                ...
                plt.hist(
                    [x for x in data if lower_bound <= x <= upper_bound],
                    bins=50,
                    color=color,
                    alpha=0.7,
                )
