...
# transparent box
blank = patches.Rectangle((0, 0), 1, 1, fc="white", ec="white", lw=0, alpha=0)
        ...
        if len(this_remaining_irrep_indexes) == 1:
            ...
            for i in this_remaining_irrep_indexes[1:]:
                ...
                if compare_lower <= this_val_lower and compare_upper >= this_val_upper:  # set new bounds on this_value
                    ...
                    and compare_lower >= this_val_lower
                    and compare_upper >= this_val_upper
                ):
                    overlap = True
                elif (
                    compare_lower <= this_val_lower
                    and compare_upper <= this_val_upper
                    and compare_upper >= this_val_lower
                ):
                    overlap = True
                if overlap:
...
class PlottingHandler:
    ...
    def correlator_plot(self, df, ptype=0, op1=None, op2=None, color_index=0, label=None):
        ...
        plt.errorbar(
            x=df["aTime"],
            y=df["FullEstimate"],
            yerr=df["SymmetricError"],
            linewidth=0.0,
            elinewidth=1.5,
            capsize=5,
            color=psettings.colors[color_index],
            marker=psettings.markers[color_index],
            label=label,
        )
        if ptype == 0:
    ...
    def sigmond_corrfit_plot_and_save(
        self,
        df,
        fit_result_info,
        Nt,
    ...
    def summary_plot(
        ...
        if index == ndatasets - 1:
            ...
            if reference:
                ...
                xlabel = (
                    rf"$\Delta E_{{lab}}/m_{{{latex_rest_mass}}}$"
                    if shift
                    else rf"$E_{{\textup{{cm}}}}/m_{{{latex_rest_mass}}}$"
                )
            else:
    ...
    def include_additional_plots(
        self, leftplotfile, rightplotfile, index=0
    ):  # add table of estimates?, list of correlators?
        """Add a subsection with two plots to the LaTeX document."""
        ...
        with self.doc[index].create(pylatex.Figure(position="H")) as thisfig:
            if os.path.exists(leftplotfile) and os.path.exists(rightplotfile):
                with self.doc[index].create(
                    ...
                    left_fig.add_image(
                        leftplotfile,
                        width=pylatex.NoEscape(llinewidth),
                ...
                with self.doc[index].create(
                    ...
                    right_fig.add_image(
                        rightplotfile,
                        width=pylatex.NoEscape(rlinewidth),
                        placement=pylatex.NoEscape("\centering"),
                    )
